...
def test_transform_compact_mode_filters_steps(sample_eph_steps):
    """Test compact mode filters to last 10 steps."""
    from nomnom.canonicalize.sankey import transform_eph_to_sankey

    # Create more steps to test filtering (13 steps total)
    extended_steps = []
    for i in range(10):
        extended_steps.append(
            (
                [],
                {f"Work {i}": CountData(nominations=5, ballot_count=4, points=300)},
                [f"Work {i}"],
            )
        )
    ...
    # Should only have nodes from last 10 steps
    # Last 10 steps: indices 3-12 (steps 10-12 are the sample_eph_steps)
    # Those steps have: 7 works eliminated + 3 finalists = some subset
    # Let's just verify we have fewer nodes than full mode would produce
...
def test_transform_first_appearance_correct_in_compact_mode(sample_eph_steps):
    """Test Fix #12: first_appearance calculated from full dataset before filtering."""
    from nomnom.canonicalize.sankey import transform_eph_to_sankey

    # Create extended dataset: works appear at different times
    extended_steps = []

    # Step 0: Work X appears first
    extended_steps.append(
        (
            [],
            {
                "Work X": CountData(nominations=5, ballot_count=4, points=300),
                "Work Y": CountData(nominations=4, ballot_count=3, points=240),
            },
            ["Work Y"],
        )
    )

    # Steps 1-9: Work X continues, new works added
    for i in range(1, 10):
        extended_steps.append(
            (
                [],
                {
                    "Work X": CountData(nominations=5, ballot_count=4, points=300),
                    f"Work {i}": CountData(nominations=3, ballot_count=2, points=180),
                },
                [f"Work {i}"],
            )
        )

    # Step 10: Work X and Work Z
    extended_steps.append(
        (
            [],
            {
                "Work X": CountData(nominations=5, ballot_count=4, points=300),
                "Work Z": CountData(nominations=2, ballot_count=2, points=120),
            },
            ["Work Z"],
        )
    )

    # Step 11: Only Work X remains (finalist)
    extended_steps.append(
        (
            [],
            {"Work X": CountData(nominations=5, ballot_count=4, points=300)},
    ...
    # Use compact mode (last 10 steps = steps 2-11)
    result = transform_eph_to_sankey(extended_steps, finalists, mode="compact")

    nodes = result["nodes"]

    # Find Work X nodes
    work_x_nodes = [n for n in nodes if n["name"] == "Work X"]
    assert len(work_x_nodes) > 0

    # CRITICAL: Work X first appeared at step 0, even though compact mode
    # only shows steps 2-11. first_appearance must be 0, NOT 2.
    for node in work_x_nodes:
        assert node["first_appearance"] == 0, (
            f"Work X first_appearance should be 0 (from full dataset), got {node['first_appearance']}"
        )

    # Work Z first appeared at step 10
    work_z_nodes = [n for n in nodes if n["name"] == "Work Z"]
    if work_z_nodes:  # Work Z is eliminated at step 10, should be in visible window
        assert work_z_nodes[0]["first_appearance"] == 10
...
def test_step_values_are_zero_based_compact_mode(sample_eph_steps):
    ...
    # Create 13 steps total so compact mode filters to last 10 (indices 3-12)
    extended_steps = []
    for i in range(10):
        extended_steps.append(
            (
    ...
    assert min(steps) == 0
    # Maximum step should be number of visible columns minus 1 (10 visible steps = 0..9,
    # plus potentially a synthetic final step)
    assert max(steps) <= 10  # 10 visible steps (indices 3-12) + possibly 1 synthetic
...
def test_display_step_preserves_absolute_index_compact_mode(sample_eph_steps):
    ...
    # 13 steps total; compact shows last 10 (indices 3-12), first_visible_idx=3
    extended_steps = []
    for i in range(10):
        extended_steps.append(
            (
    ...
    nodes = result["nodes"]
    first_visible_idx = 3  # max(0, 13 - 10) = 3
